 ...
 *
 * **Its OWN**, never the tailscale plugin's (spec 2026-09-16 § 4): the two
 * plugins drive one binary but are two products — different id, name, labels
 * and docs targets — and an install of one must not describe the other.
 *
...
if ("error" in parsed) throw new Error(`@subshell-ai/plugin-headscale has an invalid manifest: ${parsed.error}`);
