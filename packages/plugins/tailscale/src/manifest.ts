 ...
 * platform" and the two sudo commands are rendered from these bytes on a
 * machine that has never seen Tailscale.
 */
const parsed = parseManifest(pkg);
if ("error" in parsed) throw new Error(`@subshell-ai/plugin-tailscale has an invalid manifest: ${parsed.error}`);
