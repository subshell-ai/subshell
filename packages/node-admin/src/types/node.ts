 ...
 *
 * This is also the contract the node's OWN loopback dashboard mirrors: its
 * `/api/nodes/:id` answers this shape about the machine the page is running
 * on, which is what lets the cards in this package serve both surfaces with
 * one copy. Fields the node cannot know about a REMOTE machine (`shares`) or
 * that mean nothing locally (`held`) stay optional/null there.
 */
