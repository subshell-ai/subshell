...
import { LINGER_COMMAND, type PersistenceFix, persistence } from "../lib/persistence";
import type { NodeDetail, NodeRuntime } from "../types/node";
import { Badge } from "../ui/badge";
import { CopyableValue } from "../ui/copyable-value";
import { Fact, FactCard } from "./fact-list";
 ...
 */
export function NodeRuntimeCard({ node, local = false }: { node: NodeDetail; local?: boolean }): JSX.Element | null {
  const runtime = node.runtime;

  if (!runtime) return null;

  const kills = runtime.service.paneSafety !== "keeps";
  // Named with the node's OWN name on the plane — "…if nobody logs in to
  // blade-01" — because that page is about a machine the reader is not
  // sitting at. On the node's own dashboard the reader IS at that machine,
  // and the same library names it "this machine".
    ...
    <FactCard title="Runtime">
      ...
      {!runtime.supervised && (
        <p className="col-span-full text-muted-foreground text-sm">
          Nothing on {local ? "this" : "that"} machine is supervising this node, so exiting would stop it rather than
          restart it. Restart it where it was started.
        </p>
      )}
      {kills && runtime.supervised && (
        <p className="col-span-full text-sm text-warning">
          This node's service definition would close every subshell running {local ? "here" : "there"} when the node
          stops or restarts; reinstall the definition on {local ? "this" : "that"} machine to fix this.
        </p>
      )}
    </FactCard>
  );
