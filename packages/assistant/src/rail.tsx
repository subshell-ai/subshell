...
export interface RailSection {
  ...
  /** A destructive section: the label sits in the destructive token and its
      hover wash is a faint destructive one, the SPA's own destructive-item
      treatment (`actions-menu.tsx`). Never given the active gradient — a
      destructive act is not a place you are AT, it is a door you OPEN. */
  danger?: boolean;
}
    ...
    <nav aria-label="Main" className="flex w-[200px] shrink-0 flex-col gap-1 border-border border-r p-2">
      {props.sections.map((section) => (
        <button
          ...
          className={props.active === section.id ? ACTIVE : section.danger === true ? DANGER : IDLE}
