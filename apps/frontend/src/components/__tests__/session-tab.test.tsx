...
/**
 * Mounts `<SessionTab>` inside a dock context holding `panes`, with the fake
...
describe("SessionTab", () => {
  ...
  it("shows the compact waiting marker for a waiting pane", () => {
    renderTab([pane({ sessionWaitingSince: "2026-08-30T10:00:00.000Z" })], []);
  ...
  it("shows no marker for an exited pane with a stale stamp (isWaiting agrees)", () => {
    renderTab([pane({ sessionWaitingSince: "2026-08-30T10:00:00.000Z", sessionAlive: false })], []);
    expect(screen.queryByRole("img", { name: "waiting for you" })).toBeNull();
  });
