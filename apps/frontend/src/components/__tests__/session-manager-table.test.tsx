...
/** A full SessionView with overridable fields — the fixture idiom from the
 *  sibling list/picker tests (existing-session-list.test.tsx). */
...
/**
 * The table renders a name `<Link>` to `/sessions/$id` and an actions menu
 ...
 * idiom of the session-card/session-actions-menu tests.
 */
async function renderTable(sessions: SessionView[]) {
  const client = new QueryClient({ defaultOptions: { queries: { retry: false } } });
  const rootRoute = createRootRoute();
  const indexRoute = createRoute({
    getParentRoute: () => rootRoute,
    path: "/",
    component: () => <SessionManagerTable sessions={sessions} />,
  });
...
function timeCells(sessionName: string): string[] {
  const row = screen.getByText(sessionName).closest("tr");
  const cells = Array.from(row?.querySelectorAll("td") ?? []);
  return [4, 5].map((i) => cells[i]?.textContent ?? "");
}

describe("SessionManagerTable time columns (spec §5.6 nodeOffline posture)", () => {
  ...
  it("keeps '—' semantics for an offline row with no timestamps at all", async () => {
    restore = mockFetch();
    const ghost = makeSession({ id: "g2", name: "ghost-null-times", nodeOffline: true });
    await renderTable([ghost]);
