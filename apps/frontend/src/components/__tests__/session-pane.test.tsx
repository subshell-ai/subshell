 ...
 * gains the same accessory bar the session page shows on touch devices.
...
describe("SessionPane touch key bar", () => {
  it("renders no bar for a fine pointer even when the pane is active", () => {
    const spy = stubCoarsePointer(false);
    renderPane(<SessionPane pane={paneRow()} active showKeyBar onRestart={() => {}} onRemovePane={() => {}} />);
  ...
  it("renders no bar for an inactive pane on a touch device", () => {
    ...
    renderPane(
      <SessionPane pane={paneRow()} active={false} showKeyBar={false} onRestart={() => {}} onRemovePane={() => {}} />,
    );
    expect(screen.queryByRole("toolbar", { name: "Terminal special keys" })).toBeNull();
    spy.mockRestore();
  });
  ...
  it("an exited pane keeps the log-tail panel — no bar, no terminal", () => {
    ...
    renderPane(
      <SessionPane
        pane={paneRow({ sessionAlive: false })}
        active
        showKeyBar
        onRestart={() => {}}
        onRemovePane={() => {}}
