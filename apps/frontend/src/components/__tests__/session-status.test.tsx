...
import { RowStatusBadges } from "@/components/session-status";
import type { SessionView } from "@/types/session";

/** A full SessionView with overridable fields (harness copied from existing-session-list.test.tsx). */
...
describe("RowStatusBadges", () => {
  ...
  it("replaces BOTH chips with 'node unreachable' when the node is offline (spec §5.6)", () => {
    render(
      <RowStatusBadges
        session={makeSession({ nodeOffline: true, waitingSince: "2026-08-30T00:00:00.000Z", activity: "active" })}
      />,
    );
    // A downed agent makes alive/waitingSince last-known facts — neither a
    // status assertion nor a waiting claim may render (session-card.tsx
  ...
  it("shows StatusChip AND WaitingChip for an online waiting session (current behavior pinned)", () => {
    render(<RowStatusBadges session={makeSession({ waitingSince: "2026-08-30T00:00:00.000Z" })} />);
  ...
  it("leaves an online terminated row reading 'ended', never waiting", () => {
    ...
    render(
      <RowStatusBadges
        session={makeSession({ status: "terminated", alive: false, waitingSince: "2026-08-30T00:00:00.000Z" })}
      />,
    );
  ...
  it("outranks even 'ended' when the node is offline (offline-first precedence)", () => {
    render(<RowStatusBadges session={makeSession({ status: "terminated", alive: false, nodeOffline: true })} />);
    expect(screen.getByText("node unreachable")).toBeDefined();
    expect(screen.queryByText("ended")).toBeNull();
  });
});
