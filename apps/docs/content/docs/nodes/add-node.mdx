---
title: Adding a Node
description: Mint a setup key, run the install command on the target machine, and bring the node row online.
---

Adding a node is three steps: mint a setup key on the control plane, hand the target machine what the dialog renders (a one-liner for a terminal, or two values for the Subshell Client app), and watch the row come online. This page covers the terminal path, what the command does on the machine, and the one setting that causes most first attempts to fail: the server URL.

## 1. Mint a setup key

1. Open the **Nodes** page and press **Add node**.
2. Press **Create setup key**.

The mint asks for nothing, not even a name. Earlier versions opened with a "Node name" field, but that text only ever labeled the key: the one-liner named the machine from its own hostname whatever you had typed, so the question moved to the machine itself (step 3). After you press **Create setup key**, the dialog renders a copy-ready install command with the key baked in.
...
> The row you pick in the dialog's **Address the node dials** dropdown is the address the node will dial forever. Choose an address the new machine can actually reach, usually your VPN or tailnet address, before copying: the choice rides along in the command (a `server=` argument the server accepts only for addresses it already trusts) and is baked into the script when the machine downloads it. A remote machine pointed at `localhost` enrolls, then dials itself and stays offline. If the instance knows no other address, the dropdown has only that one row, so set the base URL first. See [Networking](/server/networking).

## 3. What `subshell setup` does
...
- **A node started by hand dies when you close the SSH session**: that is `subshell run`, the foreground daemon. Install the background service, as step 3 describes.
