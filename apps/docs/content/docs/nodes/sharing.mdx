...
Sharing a node extends your machine's command surface to somebody else's workloads. That is the sentence to weigh before granting anything: a share does not open a folder or a view, it opens **launch**, and what a grantee launches runs on your hardware, under the OS user your agent runs as, with all the reach that implies. Only the machine's owner can see what is running there — which is exactly why the consequences deserve their own page.
...
| **edit** | Everything above, plus configure the node: re-checks (re-probing for agent CLIs), the service controls and log reads on [the node's page](/nodes/managing-the-agent), debug-logging flips, and updates. |
