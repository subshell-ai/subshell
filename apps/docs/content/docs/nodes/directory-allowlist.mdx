...
- **The node itself** refuses every `launch` and rejects every directory check against **its own persisted copy** of the list (`allowed-dirs.json` in its data directory). A list carried inside each command would be worthless against a compromised control plane — which is the exact assumption this second check exists for.
