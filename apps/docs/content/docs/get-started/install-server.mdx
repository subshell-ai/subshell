...
The control plane ships as one self-contained binary per platform with its web UI embedded: no Bun, no checkout, no separate frontend to serve. Published targets are `linux-x64`, `linux-arm64`, `darwin-arm64` and `darwin-x64` (Intel Macs); anything else is refused by name, not silently mis-resolved. Pick the door that fits the machine:
