...
If the machine has no terminal, the dialog's reveal switches from **Terminal** to **Desktop App** and shows two values instead of a command: the server address and the setup key. In Subshell Client, paste both on its **Register This Machine** step, and it installs the node binary that ships inside the app. [Turn a desktop into a node](/nodes/desktop-node) describes that assistant screen by screen.
