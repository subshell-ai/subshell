...
- tmux, or a way to install it. `init` checks for tmux before it writes anything; an interactive run whose Mac lacks it can install it on the spot with Homebrew, with MacPorts if the machine has that instead, and by installing Homebrew itself if it has neither. The rungs that escalate ask for your admin password. Declining ends `init` with nothing written, so installing tmux and re-running is the whole fix.
