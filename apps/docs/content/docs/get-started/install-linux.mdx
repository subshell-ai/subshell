...
- tmux, or a way to install it. `init` checks for tmux before it writes anything and refuses when it is missing; an interactive run can install it on the spot with `apt-get` or `dnf`.
...
3. Answer the last question, which decides how the server runs:
   ...
   Choose **Yes**. This installs a per-user systemd service, with no sudo. `--yes` and non-interactive runs take Yes automatically.
