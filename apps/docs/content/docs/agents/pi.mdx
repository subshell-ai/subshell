...
Because MCP arrives in pi's world as an extension, the setup is two labeled steps, both shown in the MCP section of the pi preset editor, both copy-paste ready — the editor embeds your deployment's real resolved launch, so the values you paste are the correct ones. Run them **once on each machine where pi panes run**:
