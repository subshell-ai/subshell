...
> This installs the CLI the plugin drives, not a plugin. Plugins themselves are installed at Settings → Plugins and decide what Subshell can launch at all; see [Install a plugin](/agents/plugin-registry).

## See also

- [Install a plugin](/agents/plugin-registry): installing plugins, the other and more privileged act
- [Add a node](/nodes/add-a-node): enrolling the machines these CLIs need to reach
- [Audit log](/server/audit-log): where installs and refusals are recorded
- [Security model](/concepts/security-model): the trust posture these limits sit inside; the full accounting is in [docs/security.md §11.10 on GitHub](https://github.com/subshell-ai/subshell/blob/main/docs/security.md)
