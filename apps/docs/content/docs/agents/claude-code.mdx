...
- **Conversation identity.** The `SessionStart` hook reports the transcript id whenever the in-pane session changes (`/clear`, `/resume`, `/fork`), so Subshell always knows which conversation to resume.
