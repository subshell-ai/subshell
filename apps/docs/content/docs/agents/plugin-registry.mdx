...
A node does still need the real binary the plugin drives. What the store decides is which agents the instance *offers*; each machine's detection decides where they are *launchable* — see the [Agents overview](/agents).
...
> [!note] Plugin credentials, honestly stated
...
- [Writing a Harness Plugin](/develop/harness-plugin) and [Publishing a Plugin](/develop/publish-a-plugin) — the author's side of this door
