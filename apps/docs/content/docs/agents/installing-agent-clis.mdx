...
- **It is live and audited.** Installer output streams to the page line by line while it runs. When it exits, the result, exit code, and output are reported, and the agent is re-detected so the row shows its new state immediately. The run is written to the [Audit Log](/server/audit-log) with the admin who started it. No credential appears in the audit line.
...
> [!note] Not the same as installing plugins
...
- [Adding a Node](/nodes/add-node): enrolling the machines these CLIs need to reach
