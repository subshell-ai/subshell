...
The editor does not make you fill anything in — the command it shows embeds this deployment's real resolved `subshell mcp` launch, ready to copy and paste. Run it **once on each machine where Hermes panes run**: the entry lands in that machine's `~/.hermes/config.yaml`, so the control-plane host and every node that hosts Hermes subshells wants its own.
