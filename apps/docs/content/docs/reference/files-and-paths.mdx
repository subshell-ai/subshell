...
There are two homes on the control-plane host and two on a node, with a small set of rules over all of them: directories that hold credentials are `0700`, the files inside are `0600`, and because a creation mode alone depends on the umask, the modes are re-asserted after the write. The defaults below are spelled the same on Linux and macOS, and the homes sit under your user's home directory. A variable or a flag can move a path; each entry names the override.
...
- [Logs & Debugging](/server/logs-debug): reading the capped logs from the dashboard.
