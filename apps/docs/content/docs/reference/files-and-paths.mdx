...
description: Everything Subshell writes to disk, with config homes, data dirs, credentials, logs, the permission on each, and why several of these files are secrets.
---

There are two homes on the control-plane host and two on a node, with a small set of rules over all of them: directories that hold credentials are `0700`, the files inside are `0600`, and because a creation mode alone depends on the umask, the modes are re-asserted after the write.

The defaults below are spelled the same on Linux and macOS, and the homes sit under your user's home directory. A variable or a flag can move a path; each entry names the override.
...
| --- | --- | --- |
| `subshell.db` (+ `-wal`, `-shm`) | n/a | The SQLite database: accounts, sessions (hashed), subshells, nodes, audit rows, and channel ciphertext. The server stores and forwards sealed posts it cannot read. |
| `logs/server.log` | `0600` in a `0700` dir | The server's own log: one JSON-lines file, capped at 200 KB (204,800 bytes). It is replaced when full, never rotated, on every platform. HTTP request lines land here only while debug logging is on. A request path can carry a secret, which is why the file is 0600 and readable over HTTP by cookie-admins only. |
...
| `node-signing.json` | `0600` | The control plane's command-signing keypair (ES256). Every command a node receives is verified against its public half, so a compromise of this file is a compromise of every enrolled node. It is kept outside the database deliberately. |
| `node-encryption.json` | `0600` | The control plane's link-encryption keypair (X25519), generated on first use. It is a second key beside the signing one, deliberately not derived from it: one key, one job. Every node pins its public half (`controlEncryptPublicKey`) at enroll or first register, and every `/ws/node` handshake proves the plane against it. Like `node-signing.json` it is outside the database, and an unreadable file is refused, never silently regenerated. Replacing it breaks every node's pinned server identity at once, the same class of consequence as rotating the signing key. It is repaired per node by rotating the node's key: rotation clears the row's pin so the link can re-pair through `register`, and because rotation also disables the old bearer, the operator must then run `subshell configure --key <new>` on the machine and restart the agent. |
| `vapid.json` | `0600` | The browser-push keypair, generated on first use. |
| `plugins/` | n/a | The one plugin store. One install here arms every node, and a node holds no plugins of its own. `.seeded` beside the plugin directories is the completion marker: built-ins are seeded once and never re-seeded on emptiness, because an empty store is an operator who uninstalled everything, not a gap to repair. |
...
| `backups/subshell-v<version>-<UTC stamp>.db` | `0600` | Single-file database snapshots (`VACUUM INTO`), taken before every update and by `subshell-server backup`. The version is in the name because the first question about a backup is which server wrote it. `SUBSHELL_DB_BACKUPS_KEEP` prunes; `0` keeps everything. |
...
| Path | Mode | What it is |
| --- | --- | --- |
| `config.json` | `0600` | The node's whole life: which plane, its node id, its bearer key (this file is that key's only home), the pinned control public keys (the command-signing JWK, and `controlEncryptPublicKey`, the plane's link half), the node's own link keypair `encryptKeyPair` (its private half, minted at `enroll`, or at first `register` for a node predating the encrypted link), the data dir, the name, and optionally the pane-log retention window. `subshell status` never echoes the secrets, `--json` included. Written by `enroll`; `configure --server` rewrites only the address and keeps the identity (both pin halves included); `configure --key` rewrites only the stored key; the loopback dashboard's retention card rewrites only the retention window. |
| `daemon.lock` | `0600` | Local-liveness record (`pid`, `startedAt`, `nodeId`, `lastTickAt`) refreshed on every heartbeat tick and removed on every exit path. `subshell status` reads it, so the command never has to dial the plane. It is for observability, never authority. |
| `logs/agent.log` | `0600` in `0700` | The node's own log: the same JSON lines and the same 200 KB replaced-when-full cap as the server's. Deliberately under the config home rather than the data dir: logging starts before `config.json` is read, and a machine that cannot load its config is exactly the one you need the log for. |
| `data/` (default; `--data-dir` moves it) | `0700` | Runtime state, below. |
| `data/identity.json` | `0600` | The node's channel keypair (P-256, for sealed delivery). Fail-closed: a present-but-corrupt file is moved aside, never silently rotated, because overwriting it would orphan sealed history. |
...
| `data/maintenance.json` | `0600` | This machine's mirror of the maintenance flag: `{ on, changedAt }`, written atomically by whichever end set the flag. Read fail-closed. |
...
> `config.env` holds the cookie-signing secret, `config.json` holds the node key and the node's link private half, `node-signing.json` holds the key that rules every enrolled machine, and `node-encryption.json` holds the keypair every machine uses to recognise its plane on the wire. A pane log holds whatever was typed at that terminal, pasted tokens included. None of it is encrypted at rest: the key would sit on the same disk under the same user that can already read the file. The permissions and your filesystem are the control. The full accounting lives in the [threat model](https://github.com/subshell-ai/subshell/blob/main/docs/security.md).

## Other pages

- [Backups](/server/backups) and [Update the server](/server/updating): the transaction that uses `backups/` and `update/`.
- [Logs and debugging](/server/logs): reading the capped logs from the dashboard.
- [Restrict directories](/nodes/directory-allowlist): `allowed-dirs.json` from the operator's side.
- [Environment variables](/reference/environment-variables): every lever that moves a path above.
- [Security model](/concepts/security-model): what each file exposes if read.
