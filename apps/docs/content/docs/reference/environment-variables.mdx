...
| `SUBSHELL_NODE_ARTIFACTS_DIR` | `<data dir>/node-artifacts` | Where `GET /api/downloads/node/*` serves prebuilt agent binaries from, and where `release:cli-node` publishes to. The release pipeline counts an empty value as unset; the running server does not — an empty string resolves against the process's working directory — so **unset the variable, never empty it**. |
...
| `SUBSHELL_SERVER_SKIP_TMUX_CHECK` | unset | Skips the tmux presence check in `init`, `configure` and `service install` — the server-side twin of the agent's `SUBSHELL_CLIENT_SKIP_TMUX_CHECK`. |
...
| `SUBSHELL_DEBUG_LOGGING` | off | Forces debug lines into the agent's own log file and makes the plane's logging switch refuse **while armed** — the agent's rule is exactly `1` or `true`, so a set-but-falsy `SUBSHELL_DEBUG_LOGGING=0` does not arm it. Without it, the persisted `debugLogging` field in `config.json` decides. |
