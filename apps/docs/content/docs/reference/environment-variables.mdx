...
| `SUBSHELL_NODE_ARTIFACTS_DIR` | `<data dir>/node-artifacts` | Where `GET /api/downloads/node/*` serves prebuilt agent binaries from, and where `release:node` publishes to. The release pipeline counts an empty value as unset; the running server does not — an empty string resolves against the process's working directory — so **unset the variable, never empty it**. |
