...
- **`MIN_NODE_VERSION`** — currently `0.9.0`. Raise it in the same commit as a protocol bump that changes what a node must do or report, and raise the node package to the same value alongside, so the refusal an operator sees always names a version that exists.
