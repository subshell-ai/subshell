...
- **`MIN_AGENT_VERSION`** — currently `0.9.0`. Raise it in the same commit as a protocol bump that changes what a node must do or report, and raise the node package to the same value alongside, so the refusal an operator sees always names a version that exists.

The current floor rides protocol 10: a node below `0.9.0` speaks no `update`, which is the one thing a refused node must be able to hear — the plane holds such a socket precisely so it can still be rescued from a browser, and a node too old to know the command answers `unsupported`, which the route turns into "update this node by hand".
