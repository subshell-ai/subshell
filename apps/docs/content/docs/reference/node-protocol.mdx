...
- **`MIN_NODE_VERSION`** — currently `0.16.0`. Raise it in the same commit as a protocol bump, and raise the node package to the same value alongside, so the refusal an operator sees always names a version that exists.
