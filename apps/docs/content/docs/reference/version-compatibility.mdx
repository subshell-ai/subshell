...
- The node's row goes back to offline and carries the refusal's reason — "needs an update, below floor" or "protocol mismatch" — with the versions it reported. Since protocol 14 there is a third: a legacy row that sees a plaintext `ready` which would otherwise pass both gates holds the node under a pairing reason — that `ready` is the downgrade attempt the encryption handshake exists to refuse, and the held node's self-claimed identity is never written onto the row. The [Node Protocol](/reference/node-protocol) handshake section walks the `register` self-heal that ends it.
