...
`subshell-server` is the control plane's single binary: run without a subcommand it boots the server, and run with one it is a management tool. This page mirrors the binary's own usage text: the flags below are exactly the flags it parses, and anything else is a usage error.
