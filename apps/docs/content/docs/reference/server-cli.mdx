...
A key the environment already sets is refused rather than masked — writing it would be a success report for a change the next boot undoes.
...
- [subshell CLI](/reference/node-cli) — the node agent's binary, similar shape, different exit-code convention.
