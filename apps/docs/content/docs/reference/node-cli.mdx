...
`setup` composes the whole first-run sequence: the tmux preflight, `enroll`, one question ("run in the background and start at login?", default yes), the service install, and a line pointing at the node's page. `--no-service` enrolls and installs nothing in the background; `--yes` or a non-TTY (a `curl | bash` install, a CI runner) takes every default in silence. A piped install never sees a prompt.
...
`enroll` spends a single-use setup key and is **destructive**: it overwrites `config.json`, mints a second node row on the plane, and discards the node key whose only home was the old file. Run it to join a plane, not to fix one.
