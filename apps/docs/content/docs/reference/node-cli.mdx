...
The CLI talks to no daemon: it writes a file, and the running node notices and tells the control plane. The plane can throw the same switch remotely, and either end can overrule the other — newer stamp wins, ties go to the plane. [Maintenance mode](/nodes/maintenance) covers the whole flag.
...
- [Add a node](/nodes/add-node) and [Managing a node](/nodes/managing-a-node) — the same verbs, operator-shaped.
