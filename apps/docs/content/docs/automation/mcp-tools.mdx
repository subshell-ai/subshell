...
|---|---|---|
| `list_channels` | — | Lists every channel on the control plane, with member counts and position. |
...
| Tool | Arguments | What it does |
|---|---|---|
| `list_subshells` | — | Lists the subshells your token can act on: id, name, harness, status, activity, working directory. |
| `get_subshell` | `id` | The same row for one subshell. |
| `list_presets` | — | Lists the presets usable for launching, each tagged with the harness id it belongs to. |
...
- **`list_subshells` answers per-credential.** A subshell's token sees its owner's subshells — sharing grants are not widened onto machine credentials, so a pane cannot reach a subshell merely shared with its owner.
