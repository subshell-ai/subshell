...
No Bun or Node is needed on a host running these; they carry their runtime. The Intel binary is cross-built, and its release smoke runs under Rosetta, so an x86_64 Mac enrolls, runs the control plane, and self-updates like any other host.
...
The Intel images are cross-built by the same cut: `tauri build --target x86_64-apple-darwin` on the Apple Silicon runner. arm64 Linux is the one host with no GUI build, because there is no arm64 Linux runner to build or test one on. Both apps need the same glibc floor on Linux. Each app bundles the CLI binary it manages, so a desktop install needs no downloads on first run.

[Install the Control Plane](/get-started/install-server) covers which artifact to reach for. On the website, the Mac download button carries a chip menu: a browser cannot tell the two chips apart, so it asks rather than guesses.

## Universal requirements

- **tmux 3.2 or newer on every machine that runs a pane.** That is the control-plane host if it runs agents itself, and every machine enrolled as a node (see [Adding a Node](/nodes/add-node)). A machine that only hosts the UI needs nothing but the binary. Where tmux is missing on the control-plane host, the Subshell Server desktop app installs it: Homebrew on macOS, its own elevation prompt on Linux. The browser setup wizard offers the same where the server can run it, and prints the command where it cannot. A machine joining as a node (see [Adding a Node](/nodes/add-node)) is the hard case: enrollment refuses outright until tmux is present.
- **An agent CLI where you want agents**: `claude`, `opencode`, `codex`, or whichever harness plugin you launch (see [Installing Agent CLIs](/agents/installing-agent-clis)). The terminal plugin runs a plain shell, so a bare machine is still launchable without any agent installed.
...
- [Install the Control Plane](/get-started/install-server): download, verify, run
- [Adding a Node](/nodes/add-node): the node on another machine
- [Mobile and Tablet](/get-started/mobile): the companion app in detail
