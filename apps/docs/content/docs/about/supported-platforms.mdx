...
No Bun or Node is needed on a host running these; they carry their runtime. The Intel binary is cross-built and release-smoked under Rosetta, so an x86_64 Mac enrolls, runs the control plane, and self-updates like any other host.
...
The Intel images are cross-built by the same cut (`tauri build --target x86_64-apple-darwin` on the Apple Silicon runner); arm64 Linux is the one host with no GUI build: there is no arm64 Linux runner to build or test one on. Both apps need the same glibc floor on Linux. Each app bundles the CLI binary it manages, so a desktop install needs no downloads on first run. [Installing the Server](/get-started/install-server) covers which artifact to reach for; on the website, the Mac download button carries a chip menu (a browser cannot tell the two chips apart, so it asks rather than guesses).
