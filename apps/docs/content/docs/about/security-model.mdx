...
- **Any unauthenticated API call** — one guard sits in front of every API route except auth, setup status, and one deliberate exception: the sign-in page may read the instance's display name anonymously, so you can see which plane you are about to give a password to.
...
- **A forged launch on a node** — every command is signed, short-lived, single-use, and addressed to one node; the agent verifies before executing.
...
- **Network-level attackers** — no TLS enforcement and no certificate pinning; transport security is your deployment's job.
