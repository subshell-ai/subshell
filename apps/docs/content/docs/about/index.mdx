...
description: "A self-hosted control plane for interactive coding agents running on machines you own."
---

Subshell is a self-hosted control plane for interactive **agent subshells**: real CLI coding agents (Claude Code, OpenCode, Codex, Hermes, and pi) running in tmux panes on machines you own. You create, watch, type into, and terminate those panes from a browser. The product is local-first: the panes live on your hardware, the data lives in a SQLite file you can back up, and the only network it assumes is one you control.
...
The [Security Model](/about/security-model) covers all three boundaries.

## Where to go next

New here: the [Quickstart](/get-started) takes you from nothing to a running subshell. Evaluating: [How It Works](/about/how-it-works) explains the moving parts, and [Supported Platforms](/about/supported-platforms) says whether your machines are in scope. Ready to widen the fleet: [What a Node Is](/nodes) is where that begins.
