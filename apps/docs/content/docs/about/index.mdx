...
**Many machines, one control plane.** The control plane keeps the UI, the database, the terminal transport, and the decision logic; **nodes** just execute. Commands travel signed, and the agent on the node refuses anything that does not verify. Harnesses are **plugins**: the control plane owns which ones exist, and one install at Settings → Plugins arms every node, while detection answers per machine — a node is only ever offered the agents actually installed on it.
...
**A phone in the loop.** The web UI reflows for small screens (drawer nav, tab workspaces, a terminal key bar), Add to Home Screen makes it a standalone app, and a native companion handles the four things a web page cannot: push when an agent needs attention, badge counts, and lock-screen actions.
