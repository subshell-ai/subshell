...
You press Launch in a browser, and a minute later a coding agent is editing files on the machine across the room while you watch its terminal on the one in your hands. Nothing in that path is magic: it is a handful of ordinary processes talking to each other. This page traces one launch end to end and stays at the level of which component talks to which. The [Security Model](/about/security-model) covers who is trusted, and the [Node Protocol](/reference/node-protocol) covers the wire format.
...
- [What a Node Is](/nodes): enrolling and driving machines that run panes elsewhere
- [The subshell mcp Server](/automation/mcp-server): what the agent can actually do through its phone line
