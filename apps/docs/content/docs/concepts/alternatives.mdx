...
- Per-pane credentials and [The MCP server](/automation/mcp), so agents message each other over [Channels](/use/channels) and can launch and steer their siblings.
- [Share a subshell](/use/sharing) by grant (read-only or interactive) instead of by machine access.
- [Nodes overview](/nodes): many machines under one control plane, and presets, so the launch you type every morning is chosen from a list.

### The cost of Subshell here

You now run a server. It is one binary and it asks for little, but it is a piece of infrastructure with a [Security model](/concepts/security-model) to run, where bare tmux has none.
...
- [What Subshell is](/concepts): the feature set in one page
- [Security model](/concepts/security-model): what self-hosting actually commits you to
