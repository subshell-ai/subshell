...
It prints what the server *would* boot with, without booting anything: the version line, where `config.env` lives, each setting with the layer its value came from (process environment, file, or default), `tmux = NOT FOUND: install tmux (apt install tmux / brew install tmux)` when tmux is missing, the resolved MCP entrypoint, the `node artifacts = 2/3 published` line, whether an admin account exists yet, whether the configured port is listening, and what the service definition says — including the line `teardown keeps panes = NO: this definition predates the fix; reinstall it before stopping or restarting`. The auth secret appears masked, never in full. `status --json` gives the same facts for scripts. What each line means is in [Logs & Debugging](/server/logs-debug).
...
**Fix.** Use passkeys on the canonical address and put other devices on it too (add the name to `TRUSTED_ORIGINS`, then browse there). Be deliberate about changing `APP_BASE_URL` itself: it **moves** the relying-party ID, and existing passkeys stop working on the old address — including the Subshell Server desktop app's own window, which is pinned to loopback. Adding extra origins to the allowlist does not have that effect and is the safe lever for "also reachable at".
...
**Cause.** The address in the install command is baked in and dialed forever. If the instance's base URL is loopback, the remote machine dutifully dials *itself* and sits there connected to nothing. The Add-node dialog warns before you copy the command: `APP_BASE_URL points at loopback (` — the resolved URL follows — `). A remote node cannot dial this machine from itself; replace the host with this machine's VPN/LAN address (or set APP_BASE_URL).`

**Fix.** Set `APP_BASE_URL` (or at least the baked URL in the command) to an address the node can actually reach — usually your VPN or tailnet address — then mint a fresh key and rerun the command; the old key is single-use. To diagnose an already-enrolled node, `subshell status --probe` on the machine dials the stored address and names it — but read its own warning first: the probe opens a live node socket and the control plane keeps the newest connection, **so it kicks any agent running elsewhere for that node**. Only probe when no other copy is running. And if you started the daemon by hand with `subshell run`, it died with your SSH session — that verb is foreground on purpose; install the background service instead ([Adding a Node](/nodes/add-node) walks both).
...
| `subshell: could not reach $SERVER; nothing was installed ($DEST untouched).` | The machine never reached the server at all — wrong address, firewall, or the VPN was down. | Reach the address by hand (`curl -I`) and retry. |
...
**Symptom.** Subshell Server's assistant says `this build ships no server binary` (or, when it falls back to an installed one: `no server binary has been found yet`).
