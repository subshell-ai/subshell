...
| `desktop-client-vX.Y.Z publishes no darwin-x64 bundle` | That release predates (or was cut without) the Intel image. | Pin a newer release with `SUBSHELL_CLIENT_VERSION`, or take the current one-liner from the [download page](https://subshell.sh) ([Supported Platforms](/about/supported-platforms)). |
