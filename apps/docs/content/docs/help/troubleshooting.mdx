...
| `subshell: Intel Macs are not supported: no node binary is published for darwin-x64.` | Intel Macs are not a target for any component. | Run the node from a checkout on that machine, or use another host ([Supported Platforms](/about/supported-platforms)). |
