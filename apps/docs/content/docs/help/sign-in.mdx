...
1. Browse from an address already on the list, or add the address you actually use. **Settings → Networking** has the trusted-origins field in its Addresses card, and from a terminal it is `subshell-server configure --trusted-origins <origin,origin>` (see the [subshell-server CLI](/reference/server-cli)).
