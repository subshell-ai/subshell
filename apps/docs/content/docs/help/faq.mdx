...
Yes — free as in the deal, not as in a trial. No time limit, no user cap, no feature clock, no license key. Subshell is dual-licensed: everything under `apps/server/` — the control plane — is **AGPL-3.0-only**, and everything else (the node agent, the client apps, the shared packages) is **Apache-2.0**, deliberately permissive so you can write plugins, embed the agent, and build on the protocol without inheriting copyleft. The AGPL bites in exactly one direction: run a *modified* control plane as a network service and you owe its source. Building an API client is copyleft-free too — the license carries an explicit type-surface exception. The short version for API users is on [the REST API page](/automation/rest-api), and the full terms are the [LICENSE section of the README](https://github.com/subshell-ai/subshell#license).
...
Intel Macs are not built for any component — the install script refuses them by name — and the desktop apps ship for Apple silicon macOS and `amd64` Linux only. Both choices are about what can be honestly tested: every Linux artifact builds on Ubuntu 24.04, which sets a **glibc 2.39 floor** (Ubuntu 24.04 / Debian 13 or newer for every component), and there is no native arm64 Linux runner to build a GUI on. The control plane and node agent do publish `linux-arm64`, so an arm64 Linux *headless* host is fine. [Supported Platforms](/about/supported-platforms) has the matrix.
