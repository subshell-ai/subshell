...
## Why no Intel Mac, or arm64 Linux, desktop builds?

The CLI binaries cover Intel Macs (`darwin-x64`, cross-built and release-smoked under Rosetta); what Intel does not get is the GUIs, which ship for Apple silicon macOS and `amd64` Linux only. The narrower desktop matrix is about what can be honestly tested: `tauri build` links the host's own webview stack, so a GUI cannot be cross-built and magic-checked the way a headless Bun binary can, and there is no native arm64 Linux runner to build a GUI on at all. On Linux every artifact builds on Ubuntu 24.04, which sets a **glibc 2.39 floor** (Ubuntu 24.04 / Debian 13 or newer for every component). [Supported Platforms](/about/supported-platforms) has the matrix.
