...
Intel Macs are not built for any component — the install script refuses them by name — and the desktop apps ship for Apple silicon macOS and `amd64` Linux only. Both choices are about what can be honestly tested: every Linux artifact builds on Ubuntu 24.04, which sets a **glibc 2.39 floor** (Ubuntu 24.04 / Debian 13 or newer for every component), and there is no native arm64 Linux runner to build a GUI on. The control plane and node CLI do publish `linux-arm64`, so an arm64 Linux *headless* host is fine. [Supported Platforms](/about/supported-platforms) has the matrix.
