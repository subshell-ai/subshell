...
Unenrolled leftovers are on the *other* machines' side: nodes still hold their keys, and there is no plane left to call — deleting them is the [node CLI](/reference/node-cli)'s business on each machine, or nothing's.
