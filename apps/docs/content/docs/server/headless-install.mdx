...
1. **tmux preflight.** The control-plane host launches its own panes through tmux, so `init` refuses before writing anything when tmux is missing — and an interactive run gets one chance to install it on the spot (`brew install tmux` on macOS, `apt-get`/`dnf` on Linux) and then continues where it left off. A refused init leaves no trace.
