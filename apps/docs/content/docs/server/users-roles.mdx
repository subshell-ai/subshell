...
- **Disable or re-enable an account.** A disabled account cannot sign in, and every credential it holds is refused. Disabling also evicts its live sessions; re-enabling is always allowed and resumes every credential the disable refused. Like the role change, you cannot do this to yourself.
