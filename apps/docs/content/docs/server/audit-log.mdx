...
- **Credentials.** System API keys created, enabled, disabled, deleted; node setup keys created and revoked. Plaintext never appears — the setup-key rows name the key's id and nothing else.
