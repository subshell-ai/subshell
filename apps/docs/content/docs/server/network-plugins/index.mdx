...
**A network plugin describes; the host executes.** A plugin builds an argument vector, parses what a vendor CLI printed, and names a secret. It never spawns a process, never writes a file, never touches `config.env`, and never reads a credential back. Every effect goes through one host member that the server owns, and that member applies the bounds:
...
- anything whose name is `sudo`, `doas` or `pkexec` is refused outright;
- the child gets a fixed allowlist of environment variables, so neither the auth secret nor the database path rides along;
- stdin is closed, output is capped at 64 KiB, and the run is bounded — 30 seconds by default, extendable to ten minutes for a package manager that needs it.
...
Every mesh daemon needs one root install, and the server has no terminal to answer a password prompt — so it never tries. Privileged steps are manifest **data** under `network.privileged`: labelled commands, each with the vendor page that explains it, rendered as a numbered list you copy and run yourself. The plugin refuses them the same way, so declaring one in `install.command` rather than `network.privileged` fails at load.
