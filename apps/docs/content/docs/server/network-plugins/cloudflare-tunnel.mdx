...
Publish this server on a public hostname you own, behind a Cloudflare Access gate that the server verifies itself.
...
> [!important] This plugin steps outside the trusted-network posture
> What makes the posture acceptable is the gate: a Cloudflare Access application in front of the hostname, whose identity assertions this server verifies itself, on every request.
...
- [Networking and addresses](/server/networking): the origin allowlist this hostname joins while the publish stands
- [Registration and enrollment](/server/registration): deciding who gets an account, which Access cannot do for you
- [Security model](/concepts/security-model): the trusted-network posture this plugin deliberately steps outside, accounted in [docs/security.md §11.13 on GitHub](https://github.com/subshell-ai/subshell/blob/main/docs/security.md#1113-network-plugins-publish-this-server-on-a-network)
