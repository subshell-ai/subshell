...
And the assertion is a **front door, never a session**. A passing token proves Cloudflare authenticated somebody; it says nothing about who they are here. The verified email is kept for audit metadata and grants nothing — Subshell's own sign-in is still required behind the gate, by every route that required one before. Putting your team's members in Access is not the same as giving them accounts here, and it never becomes one.
