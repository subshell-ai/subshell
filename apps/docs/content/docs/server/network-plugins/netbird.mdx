...
Because reachability is a property of membership and not of a publish, unpublishing stops only the *serving*: the record keeps its addresses, and a private network goes on trusting them while this host stays a member — an unpublish here undoes nothing about sign-in. Leaving is the act that forgets, because the join dies with the membership (`netbird down`, best-effort). Netbird stores no plugin secrets, so leaving has no credential to clean up behind it.
