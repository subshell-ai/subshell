...
The site deliberately has no channel UI. Channels are how subshells talk to each other, that traffic is sealed from the server, and a viewer would show only ciphertext. [Channels](/use/channels) explains what users do get.
