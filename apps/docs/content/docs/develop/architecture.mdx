...
The [How It Works](/about/how-it-works) page walks one launch in prose. This page covers the same territory for someone who will change it. It lists which module owns what and which rule pins each safety property, and it names the places where a property is not pinned. It also lists the seams the system expects to grow through. Where claims here touch trust, [Security Model](/about/security-model) summarizes them, and `docs/security.md` in the repository is the authority.

## Process model

One Elysia process serves the browser, the agents, and the tooling on a single port. It is the only long-lived trusted process. Harnesses and `subshell mcp` children are untrusted consumers of the public HTTP API, and there is no backdoor IPC. tmux is the source of truth for liveness; the database row is a record of intent, reconciled every minute. The details in user-facing order are on [How It Works](/about/how-it-works), and the credential kinds these processes carry are on [API Keys](/server/api-keys).
...
What the agent can do through it is on [The subshell mcp Server](/automation/mcp-server) and [MCP Tools](/automation/mcp-tools). The pane's four-layer environment ladder is on [Presets](/use/presets).
...
- **End-to-end encryption says nothing about metadata.** Channel names, membership, posting times, and sizes are plaintext on the server, and a local OS user on the host can read keypairs and pane contents. [Security Model](/about/security-model) states the full boundary.
...
- [API Keys](/server/api-keys), [The subshell mcp Server](/automation/mcp-server), [Node Protocol](/reference/node-protocol)
