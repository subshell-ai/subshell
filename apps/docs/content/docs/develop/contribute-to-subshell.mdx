...
Git hooks are wired by `bun install` (lefthook): `pre-commit` formats staged files, `pre-push` runs `verify-types`, `lint:check`, `lint:licenses` and `lint:design` — and **no tests** — so pushes stay fast — CI owns the test suite, so run `bun run test` yourself before pushing anything you want green on the first try.
