...
`detect.knownPaths` names install locations PATH may not reach from a service: HOME-relative entries, or absolute ones. Each is a candidate — one that does not exist costs nothing. The `install` block is what an admin can run from the browser to put the CLI itself on the control-plane host; an optional plugin like `terminal` (which needs nothing installed) omits it.
...
## Declare what you implement — it is checked
