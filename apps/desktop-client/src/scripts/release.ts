/**
 * `bun run compile:release` — the release pipeline for `apps/desktop-client`.
 *
 * Beat for beat the pipeline `apps/desktop-server/src/scripts/release.ts`
 * runs (assertBunFloor → parseScope → preflight → stage sidecar → bundle →
 * assert the bundle set → digest → publish, all-or-nothing, CLI entry behind
 * `import.meta.main`). Exactly one thing differs: the nested product. This app
 * ships the NODE AGENT (`apps/client` → `subshell`), not the server, so the
 ...
 *    on the agent side calls `resolve()` in the CHILD's cwd (`apps/client`),
 *    so a relative override lands in `apps/client/apps/desktop-client/…` — a
...
/** `apps/desktop-client` — the cwd every tauri invocation runs in. */
 ...
 * It is the same NAME `apps/desktop-server` reads, deliberately: the release
 ...
 * `apps/client`'s own pipeline: scope it to one triple, publish it into the
...
export async function stageSidecar(deps: DesktopReleaseDeps, triple: string): Promise<boolean> {
  ...
  const code = await deps.run(["bun", "run", "--cwd", "apps/client", "compile:release"], REPO_ROOT, {
...
export function hostTarget(platform: string = process.platform, arch: string = process.arch): string {
  ...
  throw new Error(
    `apps/desktop-client cannot be built on ${platform}-${arch} (buildable here: ${DESKTOP_TARGETS.join(", ")})`,
  );
}
...
export async function readVersion(): Promise<string> {
  ...
  if (!/^\d+\.\d+\.\d+/.test(version))
    throw new Error(`apps/desktop-client/package.json has no usable version: '${version}'`);
  return version;
}

/** What `tauri build` wrote for a triple, and the ONE file that is published. */
export interface BundleArtifact {
  /** What the bundler produced: a `.deb` FILE, or — on macOS — a `.app` DIRECTORY. */
  source: string;
  /** The single file handed to `digestFile`/`publishArtifacts`. */
  artifact: string;
...
/**
 * Map a bundle root to the file this pipeline publishes.
 *
 * This is the mapping a Tauri upgrade breaks — the bundler owns the layout
 * (`bundle/<dir>/…`) and derives BOTH the `.app` directory name and the `.deb`
 * file name from `productName`. Pinned by test for that reason; `tauri build`'s
 * own argv deliberately is not.
 *
 * macOS ships a tarball rather than the `.app`, because `digestFile` and
 * `publishArtifacts` take one file — and rather than a DMG, which Tauri signs
 * but neither notarizes nor staples. The `.app`'s stapled ticket is an ordinary
 * file inside the bundle and survives the archive, which is what makes an
 * offline first launch work.
 */
export function bundleArtifact(bundleRoot: string, triple: string, version: string): BundleArtifact {
  const { dir } = bundleKind(triple);
  const artifact = join(bundleRoot, dir, desktopArtifactFileName(DESKTOP_CLIENT_PRODUCT, triple, version));
  if (triple === "linux-x64") return { source: artifact, artifact, archive: false };
  return { source: join(bundleRoot, dir, `${DESKTOP_CLIENT_PRODUCT}.app`), artifact, archive: true };
}

/**
 * Reduce a built bundle to ONE publishable file, archiving the macOS `.app`.
 *
 * `--no-mac-metadata` keeps AppleDouble `._` members out: one that survives
 * into the archive breaks the extracted bundle's signature, and the failure
 * then reads as a signing bug rather than a packaging one.
 */
export async function collectArtifact(
  ...
  const { source, artifact, archive } = bundleArtifact(bundleRoot, triple, version);
  if (!archive) return artifact;
...
async function main(): Promise<void> {
  ...
  // them (`assertWorkspaceBuilt` in apps/client's pipeline). Mirroring that
