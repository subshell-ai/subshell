...
//! rule that has a home in `apps/client` — which is why every action returns
...
/// (`apps/server/src/api/nodes/enroll.route.ts`), which the CLI also
...
/// in `apps/client/src/enroll.ts` does the same, and the string this returns is
...
/// `apps/desktop-server`'s `subshell-server`, which answers `version` with a
