...
//! `apps/desktop-server`'s otherwise identical ladder:
//!
//! 1. `execLine()` in `apps/client/src/service.ts` appends the VERB: a
...
/// The systemd user unit `apps/client/src/service.ts` installs.
///
/// Pinned here rather than derived: it is the CLIENT's unit
/// (`subshell.service`), and reading `apps/desktop-server`'s
...
/// The launchd label `apps/client/src/service.ts` installs under.
...
mod exec_parse_tests {
    ...
    fn a_dev_form_install_keeps_both_tokens() {
        assert_eq!(
            parse_systemd_exec("/Users/x/.bun/bin/bun /repo/apps/client/src/main.ts run"),
            ["/Users/x/.bun/bin/bun", "/repo/apps/client/src/main.ts", "run"]
        );
    }
...
mod ladder_tests {
    ...
    // build, or `apps/desktop-server`'s `subshell-server` under a symlink.
...
mod sidecar_spec_tests {
    ...
    // app's — `apps/client/src/service.ts` SYSTEMD_UNIT_NAME / LAUNCHD_LABEL.
    #[test]
