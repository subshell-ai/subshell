...
const NameBodySchema = t.Object({
  name: t.Optional(
    t.String({ minLength: 1, maxLength: 120, description: "New session display name (max 120 chars); locks the name" }),
  ),
...
/** `PATCH /api/sessions/:id/name` — renames a session. */
export const updateSessionNameRoute = new Elysia()
  .use(contextPlugin)
  .use(authGuard)
  .use(apiModels)
  .patch(
    ...
    async ({ params, body, user, actor, principal, apiKeyPermissions, ctx }) => {
      ...
      if (actor === "session-key" && principal !== `sess:${params.id}`) {
        throw new HttpError(403, "A session token may only rename its own session");
      }
      ...
      // that empties out is invalid — unlike notes, a session must have one.
