 ...
 * the list and rejects new profiles/sessions, and re-enabling restores
 ...
 * session — this suite's calls therefore carry the signed-in cookie (the
...
describe("harness enable/disable", () => {
  ...
  it("disabling hides its profiles; re-enabling brings them back; creation is blocked", async () => {
    ...
    const session = await sessionRoutes.fetch(
      authedRequest("/api/sessions", token, {
        method: "POST",
    ...
    if (session.ok) {
      ...
      // live tmux session behind.
      const { id } = (await session.json()) as { id: string };
      await sessionRoutes.fetch(authedRequest(`/api/sessions/${id}`, token, { method: "DELETE" }));
    }
