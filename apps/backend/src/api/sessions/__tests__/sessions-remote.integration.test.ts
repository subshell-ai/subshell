...
const app = new Elysia().use(errorHandlerPlugin).use(sessionRoutes).use(uploadsRoutes);
...
describe("remote sessions over real routes (Task 14 lock-step)", () => {
  ...
  /** `POST /api/sessions` pinned to the scripted node; asserts the honest 200 shape. */
  ...
  afterAll(async () => {
    ...
    for (const id of createdSessionIds) await sessionsRepo.delete(id).catch(() => {});
  ...
  it("happy full lifecycle: launch frame on the wire, prompt delivered, nodeOffline across a detach, sweep partition", async () => {
    ...
    try {
      ...
      expect(launch.sessionName).toBe("it-remote");
      ...
      // the local twin never sized newSession either (the first attach's
      ...
      // (apps/agent/src/commands/launch.ts) without another protocol change.
      ...
      const view = async () =>
        (await (await post(`/api/sessions/${id}`, undefined, "GET")).json()) as {
          nodeId: string;
          nodeOffline: boolean;
          alive: boolean;
        };
      ...
      expect((await post(`/api/sessions/${id}/terminate`)).status).toBe(200);
  ...
  it("terminate ⇒ `kill` on the wire, row retired", async () => {
    ...
    try {
      ...
      const res = await post(`/api/sessions/${body.id}/terminate`);
      expect(res.status).toBe(200);
      expect(await res.json()).toEqual({ ok: true });

      expect(sim.cmdTypes().slice(before)).toEqual(["kill"]);
      expect(sim.cmdsOf("kill")).toEqual([{ type: "kill", sessionId: body.id }]);

      const row = await sessionsRepo.findById(body.id);
      expect(row?.status).toBe("terminated");
      expect(row?.alive).toBe(0);
      const view = (await (await post(`/api/sessions/${body.id}`, undefined, "GET")).json()) as {
        status: string;
        alive: boolean;
      };
  ...
  it("restart in place ⇒ kill, rotate, `probe_resume` first, relaunch with bestEffortLog on the wire", async () => {
    ...
    try {
      ...
      const res = await post(`/api/sessions/${body.id}/restart`);
      expect(res.status).toBe(200);
  ...
  it("delete ⇒ kill + `remove_paths` naming log, mcp and meta paths; row gone", async () => {
    ...
    try {
      ...
      const res = await post(`/api/sessions/${body.id}`, undefined, "DELETE");
      expect(res.status).toBe(200);
      expect(await res.json()).toEqual({ ok: true });

      // The three artifacts a session leaves on a node go together (the log is
      ...
      expect(sim.cmdsOf("remove_paths")).toEqual([
        {
          type: "remove_paths",
          paths: [
            `${SCRIPTED_DATA_DIR}/sessions/${body.id}.log`,
            `${SCRIPTED_DATA_DIR}/mcp/${body.id}.json`,
            `${SCRIPTED_DATA_DIR}/sessions/${body.id}.meta.json`,
          ],
        },
      ]);
      expect(await sessionsRepo.findById(body.id)).toBeUndefined();
    } finally {
      sim.detach();
    }
  });
  ...
  describe("uploads relay through the same routes (Task 12 fixture, scripted agent)", () => {
     ...
     * (`apps/agent/src/commands/write-file.ts`, pinned by its own suite):
    ...
    async function upload(sessionId: string, file: File) {
      ...
      return await app.fetch(
        authedRequest(`/api/sessions/${sessionId}/uploads`, cookie, { method: "POST", body: form }),
      );
    }
     ...
     */
    function emptyUploadRequest(sessionId: string, name: string): Request {
      const boundary = `----moteit${crypto.randomUUID().replace(/-/g, "")}`;
      const raw =
        `--${boundary}\r\nContent-Disposition: form-data; name="file"; filename="${name}"\r\n` +
        "Content-Type: application/octet-stream\r\n\r\n\r\n" +
        `--${boundary}--\r\n`;
      return authedRequest(`/api/sessions/${sessionId}/uploads`, cookie, {
        method: "POST",
        body: raw,
    ...
    it("a ZERO-BYTE upload terminates the write with one empty eof chunk and yields a real 0-byte file", async () => {
      ...
      const sim = attachScriptedNode(nodeId, { write_file: fs.handler });
      try {
        const res = await app.fetch(emptyUploadRequest(sessionId, "empty.bin"));
        expect(res.status).toBe(200);
        const json = (await res.json()) as { path: string; name: string; size: number; contentType: string };
