...
const NotesBodySchema = t.Object({
  notes: t.Union(
    [
      t.String({ maxLength: 2000, description: "Operator note for the session (max 2000 chars)" }),
...
/** `PATCH /api/sessions/:id/notes` — sets or clears a session note. */
export const updateSessionNotesRoute = new Elysia()
  .use(contextPlugin)
  .use(authGuard)
  .use(apiModels)
  .patch(
    ...
    },
    {
      ...
      detail: {
        ...
        tags: ["sessions"],
        description: "Set or clear a session note",
      },
    },
  );
