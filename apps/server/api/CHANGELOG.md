...
- [#171](https://github.com/subshell-ai/subshell/pull/171) [`c60dfe7`](https://github.com/subshell-ai/subshell/commit/c60dfe703d66cb042492192ab2c3030fe916dd28) Thanks [@theogravity](https://github.com/theogravity)! - Presets: the row action menu gains Clone preset. It opens the create dialog prefilled from the source (same agent, its env, flags, and restart policy) with the next free "(2)" name suggested.
...
- [#170](https://github.com/subshell-ai/subshell/pull/170) [`ac9ec22`](https://github.com/subshell-ai/subshell/commit/ac9ec2284b0ee2a9e5a962a2caa9332dcd02ddf9) Thanks [@theogravity](https://github.com/theogravity)! - Three security fixes from the 2026-09 documentation sweep, operator-approved.
  ...
  **The WebSocket attach-token store is capped** (audit item 8). Mints became machine-reachable in [#159](https://github.com/subshell-ai/subshell/issues/159) and the in-memory store had no limit; past `MAX_PENDING_WS_TOKENS` (10 000) the mint sweeps expired tokens first and then answers a named 503 rather than growing memory on a script-rate caller's schedule.
...
- [#176](https://github.com/subshell-ai/subshell/pull/176) [`db5951a`](https://github.com/subshell-ai/subshell/commit/db5951ae26bbc8cd4ea6a32be3114b84df786396) Thanks [@theogravity](https://github.com/theogravity)! - The sidebar's subshell-row tooltip is now a real in-page popup, so it scales with browser zoom. It used to ride the element's native `title`, which the browser paints at the SYSTEM font size; ctrl +/- grew the rail and left the reveal behind. The tooltip is the shadcn Base UI component, attached through its `render` prop so the row stays one element (nav, drag source, context-menu subject and tooltip trigger all on the same `Link`); a keyboard-focused row shows the same reveal, which `title` gave only on hover. The styled tooltip's text went one step up the scale, 13 to 14.

- [#178](https://github.com/subshell-ai/subshell/pull/178) [`ce81149`](https://github.com/subshell-ai/subshell/commit/ce81149722d6ce2b276a8941cf4f602ea8f76cbc) Thanks [@theogravity](https://github.com/theogravity)! - Users page: the per-row admin controls (role flip, password reset, disable/re-enable) fold into one ⋯ action menu like every other roster in the app, the "Manage" column header is gone, and the confirm dialogs say what they do in two sentences.
...
- [#155](https://github.com/subshell-ai/subshell/pull/155) [`6626c20`](https://github.com/subshell-ai/subshell/commit/6626c20573edfe0c94ebc5eac7c2b1affccdab06) Thanks [@theogravity](https://github.com/theogravity)! - The Nodes page is now grouped into a left-hugging tab set that speaks the same visual language as the subshells list — Tiles / List / Keys with the shared icons, where the card list, a new column-aligned table of the same machines, and the setup-key ledger each get their own address (`/nodes`, `?tab=list`, `?tab=keys`). The page subtitle reads "Nodes allow you to run subshells on other machines." On the node harness card, the Install button moved up onto the harness's own row (shrunk to just "Install"), replacing the per-row "checked … ago" stamp that used to occupy that cell.

## 0.16.1

### Patch Changes

- [#154](https://github.com/subshell-ai/subshell/pull/154) [`d305b8d`](https://github.com/subshell-ai/subshell/commit/d305b8d966f90b19886076a1a60b48fd9737703d) Thanks [@theogravity](https://github.com/theogravity)! - Terminal typing over slow links is no longer serialized at one burst per round trip: the input queue flushes coalesced keystrokes on a 30 ms window and pipelines up to 8 unacked frames. Measured on a WAN node (83 ms RTT, ~350 ms acks), twelve keystrokes typed 30 ms apart went from 687 ms to fully pipelined delivery; LAN behavior is unchanged. The pane diagnostics HUD shows Echo p50, Echo max and Oldest on their own rows with Viewers above Output, and the API-key reveal dialog wraps the key instead of overflowing.
