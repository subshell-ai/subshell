...
describe("remote subshells over real routes (Task 14 lock-step)", () => {
  ...
  beforeAll(async () => {
    ...
    // Post-inversion (Task 9) that fresh cache row plus the instance store
    // holding claude-code (setupAuthTables seeds the built-ins to the shared
    // temp data dir) is the whole gate: the node declares nothing.
  });
