...
/**
 * The bearer half of authGuard: API keys (system + subshell) authenticate the
...
// Real /api/subshells surface with the production error serializer, so a
// guard-thrown 401 carries the ApiErrorResponse body (and its message) the
// same way it does under createApp().
const restApp = new Elysia().use(errorHandlerPlugin).use(subshellRoutes);
...
describe("authGuard bearer path", () => {
  ...
  async function makeSubshell(): Promise<string> {
  ...
  it("a self-minted key cannot forge a subshell principal or a system actor", async () => {
    // The plugin's create endpoint accepts arbitrary metadata, so the GUARD
    // must distrust it: subshell-kind requires the server-written apiKeyId
    // link; system actor requires the system user's ownership.
    const sid = await makeSubshell();
    const legit = await issueSubshellToken(sid, userId);
  ...
  it('a "node" kind key 401s on REST with an explicit node-key message (spec §5.5)', async () => {
    ...
    const res = await restApp.fetch(
      new Request("http://localhost:3080/api/subshells", { headers: { authorization: `Bearer ${created.key}` } }),
    );
  ...
  it("subshell key bearer yields sess principal and its permissions", async () => {
    const sid = await makeSubshell();
    ...
    expect(status).toBe(200);
    expect(body?.actor).toBe("subshell-key");
    expect(body?.principal).toBe(`sess:${sid}`);
    expect(body?.userId).toBe(userId); // routes keep working via the OWNER's id
    expect(body?.perms).toMatchObject({ channels: ["read", "write"], subshells: ["read", "write"] });
  });

  it("subshell key whose subshell row is gone -> 401", async () => {
    const sid = await makeSubshell();
    const key = await issueSubshellToken(sid, userId);
    const row = await new SubshellsRepository(db).findById(sid);
    if (row?.apiKeyId) createdKeyIds.push(row.apiKeyId);
    await db.deleteFrom("subshells").where("id", "=", sid).execute();
    expect((await bearerGet(key)).status).toBe(401);
  });

  it("subshell key whose apiKeyId link was unlinked -> 401 (real guard)", async () => {
    // M-4c (final review): the lifecycle wave's revoke-failure fallback does
    // exactly this write (`subshells.update(id, { apiKeyId: null })`). The key
    ...
    await new SubshellsRepository(db).update(sid, { apiKeyId: null });
    expect((await bearerGet(key)).status).toBe(401);
  });

  it("requirePerm allows cookie/system actors, enforces subshell-key grants", () => {
    expect(() => requirePerm({ actor: "cookie", apiKeyPermissions: null }, "channels", "read")).not.toThrow();
    expect(() => requirePerm({ actor: "system-key", apiKeyPermissions: null }, "subshells", "write")).not.toThrow();
    expect(() =>
      requirePerm({ actor: "subshell-key", apiKeyPermissions: { channels: ["read"] } }, "channels", "read"),
    ).not.toThrow();
    expect(() =>
      requirePerm({ actor: "subshell-key", apiKeyPermissions: { channels: ["read"] } }, "channels", "write"),
    ).toThrow(ForbiddenError);
    expect(() =>
      requirePerm({ actor: "subshell-key", apiKeyPermissions: { channels: ["read"] } }, "subshells", "read"),
    ).toThrow(ForbiddenError);
  });
