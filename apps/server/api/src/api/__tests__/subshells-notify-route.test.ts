...
/**
 * `POST /api/subshells/:id/attention` (harness hook self-report, subshell-key
 * only) and `PATCH /api/subshells/:id/notify` (the ⋯-menu bell toggle, owner
 * cookie or the subshell's own key). SubshellsService.recordAttention stamps
 ...
 */
describe("subshells attention + notify routes", () => {
  ...
  // A and B are both the owner's subshells — the self-scope guard is per-row,
  ...
  beforeAll(async () => {
    ...
    idA = await mintSubshell("notify-A");
    keyA = await issueSubshellToken(idA, ownerId);
    await trackKey(idA);
    idB = await mintSubshell("notify-B");
    keyB = await issueSubshellToken(idB, ownerId);
    await trackKey(idB);
  });
  ...
  /** Creates an owned subshell row (bell ON, nothing pending) and tracks it. */
  async function mintSubshell(name: string, over: { alive?: number } = {}): Promise<string> {
  ...
  /** Tracks the key issued by issueSubshellToken for later apikey cleanup. */
  async function trackKey(subshellId: string): Promise<void> {
  ...
  async function row(id: string) {
    const found = await new SubshellsRepository(db).findById(id);
    if (!found) throw new Error(`test subshell row missing: ${id}`);
    return found;
  }

  it("attention from the subshell's own key sets waiting_since and rings", async () => {
  ...
  it("attention from a foreign subshell key → 403 (per-row self-scope)", async () => {
  ...
  it("attention on a DEAD row is a silent 200 — no stamp resurrection, no push", async () => {
    ...
    const deadId = await mintSubshell("notify-dead", { alive: 0 });
    const deadKey = await issueSubshellToken(deadId, ownerId);
  ...
  it("PATCH notify toggles the bell both ways and never clears waiting_since; foreign cookie user → 404", async () => {
    ...
    await new SubshellsRepository(db).update(idB, { waitingSince: stamp });
  ...
  it("PATCH notify from the subshell's own key is allowed for itself, not others", async () => {
    ...
    await new SubshellsRepository(db).update(idA, { notify: 1 });
    ...
    await new SubshellsRepository(db).update(idB, { notify: 1 });
