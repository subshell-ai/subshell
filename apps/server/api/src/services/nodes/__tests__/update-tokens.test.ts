...
describe("update tokens", () => {
  ...
  it("mints a url-safe `nut_` token and spends it exactly once", () => {
    const token = mintUpdateToken("n1", "linux-x64");
    ...
    expect(consumeUpdateToken(token, "linux-x64")).toBe("n1");
  ...
  it("does not accumulate: minting sweeps spent and expired records", () => {
    const spent = mintUpdateToken("n1", "linux-x64");
    consumeUpdateToken(spent, "linux-x64");
    mintUpdateToken("n2", "linux-arm64");
  ...
  it("keeps two nodes' tokens distinct", () => {
    const a = mintUpdateToken("n1", "linux-x64");
    const b = mintUpdateToken("n2", "linux-x64");
    expect(a).not.toBe(b);
    expect(consumeUpdateToken(a, "linux-x64")).toBe("n1");
    expect(consumeUpdateToken(b, "linux-x64")).toBe("n2");
  });
});
