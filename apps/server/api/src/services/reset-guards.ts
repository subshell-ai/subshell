 ...
 * ported from the desktop's pinned twin in
 * `crates/desktop-core/src/reset_guards.rs`: guards must exist on every chain
 * that deletes, because the delete plan is assembled from ambient constants
 * and a mistyped `SUBSHELL_SERVER_DATA_DIR` (`/`, `$HOME`) is a live `rm -rf`
 * with a nice message behind it. The words match the Rust file on purpose;
 ...
 * refused data dir for a binary at `~/.local/bin/subshell-server` is the
