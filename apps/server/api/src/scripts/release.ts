...
/** The plugin-embed step's one seam (mirrors the agent pipeline's `EmbedPluginsDeps`). */
...
/**
 * Bakes the built-in plugins into pane-runtime's generated module, BEFORE
 * any compile.
 *
 * Without this the server binary ships the tracked empty stub, and on a host
 * with no checkout `builtInIds()` answers nothing: boot seeds an empty
 * store, no user gets a Default profile, and every launch refuses. The agent
 * has run this step since the inversion; the server owns the store, so the
 * server must embed too.
 * @param deps - generator seam
 */
