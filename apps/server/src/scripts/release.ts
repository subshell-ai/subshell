 ...
 * `apps/server/dist/release/` with `--bytecode` (uniform, floor 1.4.0
...
/** `apps/server` — the cwd every `bun build`/embed/restore invocation runs in. */
const SERVER_DIR = resolve(SCRIPT_DIR, "..", "..");
/** The monorepo root — the default publish dir and the frontend-dist preflight home. */
...
/**
 * `bun build` argv for one target (spawned with cwd `apps/server`).
...
export interface EmbedDeps {
  /** True when `apps/frontend/dist/index.html` exists (fake fs in tests). */
...
export async function runEmbed(deps: EmbedDeps): Promise<void> {
  if (!deps.distIndexExists()) {
    throw new Error(
      "the built SPA is missing (apps/frontend/dist/index.html) — " +
...
/** Runs one `bun` subcommand argv in `apps/server` with output streamed to this console. */
 ...
 * generated one (`git checkout` from `apps/server`). A failure here is loud
  ...
  if ((await child.exited) !== 0) {
    process.stderr.write(
      "compile:release: could not restore the embedded-web stub — run " +
        "`git checkout -- apps/server/src/generated/embedded-web.ts` by hand.\n",
    );
  }
}
...
async function main(): Promise<void> {
  ...
  const result = await runRelease(
    {
      embed: {
        distIndexExists: () => existsSync(join(REPO_ROOT, "apps", "frontend", "dist", "index.html")),
