...
export function createResetView(host: AssistantHost): ResetView {
  ...
  function render(): void {
    ...
    el("reset-disclosures").textContent =
      "Enrolled remote nodes are NOT reached: their daemons and panes keep running with keys to a plane that will not exist. A subshell node on this very machine is not reached either and must be stopped from Subshell Client or `subshell service stop`. The installed server binary stays. Everything listed above is permanent.";
