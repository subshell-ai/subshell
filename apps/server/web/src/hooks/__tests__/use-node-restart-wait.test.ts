...
import { isNewAgentProcess } from "@/hooks/use-node-restart-wait";
 ...
 */
describe("isNewAgentProcess", () => {
  it("calls an unchanged startedAt the same process", () => {
    expect(isNewAgentProcess("2026-09-12T10:00:00.000Z", "2026-09-12T10:00:00.000Z")).toBe(false);
  });
  ...
  it("catches a restart that completed inside the server waiter's tolerance", () => {
    ...
    expect(isNewAgentProcess("2026-09-12T10:00:00.000Z", "2026-09-12T10:00:03.000Z")).toBe(true);
  });
  ...
  it("treats a missing baseline as a new process", () => {
    ...
    expect(isNewAgentProcess(undefined, "2026-09-12T10:00:00.000Z")).toBe(true);
  });
});
