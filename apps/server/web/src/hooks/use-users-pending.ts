...
/**
 * One row of `GET /api/users/pending` — a person a provider let through who is
 * not a member yet (spec 2026-09-24 §6).
 */
export interface PendingUserRow {
  /** better-auth user id */
  id: string;
  /** The email the provider's profile carried */
  email: string;
  /** Display name; empty for a provider arrival that carried none */
  name: string;
  /** The provider this arrival came through; null for an account with no sign-in row */
  providerId: string | null;
  /** The provider's current name, resolved live; null when it has since been removed */
