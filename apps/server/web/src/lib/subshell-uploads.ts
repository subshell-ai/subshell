...
/** Progress callback for {@link uploadSubshellFile}: bytes handed off, total bytes. */
export type UploadProgress = (sent: number, total: number) => void;

/**
 * Uploads one file into a subshell's working directory.
 *
 ...
 *
 * @param subshellId - Subshell to upload into
 ...
 */
export function uploadSubshellFile(subshellId: string, file: File, onProgress?: UploadProgress): Promise<string> {
  return new Promise((resolve, reject) => {
    const body = new FormData();
    body.set("file", file);
    const xhr = new XMLHttpRequest();
    xhr.open("POST", `/api/subshells/${encodeURIComponent(subshellId)}/uploads`);
...
export interface UploadAttempt {
  ...
  name: string;
  /** Settled result of `uploadSubshellFile` for this file */
 ...
 * could not be inserted because the subshell socket was closed (e.g.
...
export function insertionFailedMessage(paths: string[]): string {
  return `Uploaded but the subshell isn't connected, so nothing was inserted: ${paths.join(" ")}`;
}
