...
/*
 * LINGER_COMMAND, PersistenceInput, PersistenceFix and persistence() moved to
 * `@internal/node-admin/lib/persistence.ts` on 2026-09-19: the question they
 * answer — "will this machine bring the process back by itself?" — is no
 * longer the server page's alone. The node's Runtime card answers it from a
 * runtime report, and the node's OWN dashboard answers it about the machine
 * the reader is sitting at; one model, three surfaces, `machine` the
 * parameter that keeps the sentences honest. What stays here is the
 * SUPERVISION question — WHO runs the server — which speaks
 * `ServerDeployment` and belongs to this app.
 */
