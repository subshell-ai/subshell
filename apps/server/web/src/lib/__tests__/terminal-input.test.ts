...
describe("dropBrokenMouseReports", () => {
  it("drops the exact frames measured on a flick (2026-09-18)", () => {
    // xterm's momentum frames report a wheel event at coordinates the event
    // never carried; untouched, the harness printed the tail of these into
    // its prompt.
  ...
  it("takes the ESC with it — a leftover one is an Escape keypress", () => {
    // 45 momentum frames in one flick (measured), so a filter that stripped
    // only the tail would send the harness 45 Escapes instead of 45 lines of
    // garbage: quieter on screen, and worse at a prompt that cancels on ESC.
    expect(dropBrokenMouseReports(`${ESC}[<65;NaN;NaNM`)).not.toContain(ESC);
  });
