...
import { injectText } from "../subshell-frames.js";
import {
...
describe("uploadSubshellFile (XHR transport)", () => {
  it("posts multipart to the subshell route with cookies and resolves the stored path", async () => {
    globalThis.XMLHttpRequest = FakeXhr as unknown as typeof XMLHttpRequest;
    const file = new File(["hello"], "a.txt", { type: "text/plain" });
    const p = uploadSubshellFile("s 1", file);
    const xhr = FakeXhr.instances[0];
    expect(xhr.method).toBe("POST");
    expect(xhr.url).toBe("/api/subshells/s%201/uploads");
  ...
  it("reports byte progress through the callback", async () => {
    ...
    const p = uploadSubshellFile("s1", new File(["x"], "a.png"), (sent, total) => seen.push([sent, total]));
    const xhr = FakeXhr.instances[0];
  ...
  it("prefers the API's structured message on refusal, else the bare status", async () => {
    globalThis.XMLHttpRequest = FakeXhr as unknown as typeof XMLHttpRequest;
    const p1 = uploadSubshellFile("s1", new File(["x"], "a.txt"));
    FakeXhr.instances[0].respond(409, JSON.stringify({ message: "Subshell working directory is missing" }));
    await expect(p1).rejects.toThrow("Subshell working directory is missing");

    const p2 = uploadSubshellFile("s1", new File(["x"], "a.txt"));
  ...
  it("a network failure rejects with a usable message", async () => {
    globalThis.XMLHttpRequest = FakeXhr as unknown as typeof XMLHttpRequest;
    const p = uploadSubshellFile("s1", new File(["x"], "a.txt"));
