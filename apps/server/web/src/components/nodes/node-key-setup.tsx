...
export function useSetupKeyVerdict() {
  ...
  const missingNote = missingTargets.length > 0 && (
    <p className="text-amber-600 text-detail dark:text-amber-400">
      This server has no node binary for: {missingTargets.join(", ")}, and it is configured not to download one, so the
      install command 404s on those machines. Publish the binaries (run{" "}
      <code className="font-mono">bun run release:cli-node</code> from a checkout, or copy them from a cli-node-vX.Y.Z
      release into that dir), or use the Subshell Client app, which ships its own node CLI.
    </p>
  );
