...
describe("DesktopAppUpdateRow", () => {
  it("names the app, its version, and what is available", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Subshell Server 0.7.2 — v0.8.0 available. Open updates." }),
    ).toBeTruthy();
  ...
  it("goes to the Updates page, and installs nothing itself", async () => {
    ...
    const row = await screen.findByRole("button", {
      name: "Subshell Server 0.7.2 — v0.8.0 available. Open updates.",
    });
    fireEvent.click(row);
  ...
  it("is still a door when there is no update known", async () => {
    ...
    const row = await screen.findByRole("button", { name: "Subshell Server 0.7.2. Open updates." });
    fireEvent.click(row);
    await waitFor(() => expect(landedOn).toBe("/settings/updates"));
  ...
  it("gives a member the line and nothing to press, since the page is admin-only", async () => {
    setUA(SERVER_UA);
    const invocations = fakeTauri({ currentVersion: "0.7.2", availableVersion: "0.8.0" });
    renderRow(false, false);
    // The version and the dot still show — a member may quote both in a bug
    // report. What they do not get is a press that lands on a route that will
    // not render for them.
    await waitFor(() => expect(screen.getByText(/v0\.8\.0 available/)).toBeTruthy());
