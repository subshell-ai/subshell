 ...
 *
 * `local` is never here — the control-plane host's update IS the Server card
 ...
 */
export function NodesCard({ fleet }: { fleet: NodeUpdates }) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {fleet.release !== null
            ? `Nodes can be updated to ${fleet.release.version}.`
            : `No node release can be offered: ${endOnce(fleet.reason ?? "no reason was given")}`}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        {fleet.rows.length === 0 ? (
          <p className="text-muted-foreground text-sm">No machines are enrolled as nodes.</p>
        ) : (
              ...
              <li key={row.id} className="flex items-center justify-between gap-3 py-2">
                <div className="min-w-0">
                  ...
                  <p className="truncate text-detail text-muted-foreground">
                    {row.agentVersion ?? "version unknown"} · {row.target ?? "no published platform"} ·{" "}
                    {rowState(row, fleet)}
                  </p>
                  {!row.canUpdate.ok && row.canUpdate.reason !== null && (
                    <p className="truncate text-detail text-muted-foreground">{row.canUpdate.reason}</p>
                  )}
                  {nodeUpdate.failure?.nodeId === row.id && (
                    <p className="text-destructive text-detail">{nodeUpdate.failure.message}</p>
                  )}
                </div>
                <Button
                  variant="outline"
        ...
        {runError !== null && <p className="text-destructive text-detail">{runError}</p>}
