...
/**
 * Bakes the built-in plugins into `pane-runtime`'s generated module.
 *
 * A compiled agent has no `packages/plugins` beside it, so the embedded copies
 * are the only source `readBuiltIn` can answer from. Refusing here beats
 * shipping a binary that installs nothing.
 */
export async function runEmbedPlugins(deps: EmbedPluginsDeps = defaultEmbedPluginsDeps): Promise<void> {
  if ((await deps.runGenerator()) !== 0) {
    throw new Error(
...
/** The one side effect {@link runEmbedPlugins} has, as a seam. */
...
const defaultEmbedPluginsDeps: EmbedPluginsDeps = {
  runGenerator: async () => {
    const proc = Bun.spawn(["bun", "run", join("src", "scripts", "embed-plugins.ts")], {
...
/**
 * Restores the tracked empty stub.
 *
 * Generated bytes are release noise, never a commit, and this runs in a
 * `finally` so a failed build does not leave 84 KB of them in the worktree.
 */
async function restoreEmbedStub(): Promise<void> {
  const proc = Bun.spawn(["git", "checkout", "--", EMBED_STUB], {
    cwd: REPO_ROOT,
    stdout: "ignore",
    stderr: "inherit",
  });
...
async function main(): Promise<void> {
  ...
  // Bake the built-in plugins into the binary BEFORE compiling. A compiled
  // agent has no checkout beside it, so without this it ships an empty
