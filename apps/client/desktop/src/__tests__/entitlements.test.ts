...
const DESKTOP_SERVER_PLIST = join(import.meta.dir, "../../../../server/desktop/src-tauri/entitlements.plist");
...
describe("macOS entitlements", () => {
  // The same two `apps/server/desktop` pins, and for the same measured reason:
  // a `bun build --compile` binary signed ad-hoc with `--options runtime` and
  // ONLY these two runs. See apps/server/desktop/AGENTS.md for the caveat that
