 ...
 * decision. 9 substitutions total (≤ 10 budget):
 *
 ...
 *  6. `env.ts` — PORTED VERBATIM, not replaced: it already IS the pane-env
 ...
 *  7. Backend `mcp/main.ts` (top-level `runMoteMcp().catch(stderr; exit 1)`) →
 ...
 *  8. Identity semantics UNCHANGED — STOP clause checked and CLEARED:
 ...
 *  9. Dev-only additions (NOT in the ported files): `cli.ts` (command wiring,
