...
/**
 * Port-parity suite (Task 13): cloned from `apps/backend/src/mcp/__tests__/
 * tools.test.ts` with imports rehomed — the agent twin must fail exactly where
 * the backend twin fails. The last case goes one step beyond the clone: it
 * drives the REAL {@link MoteApi} over a stubbed `fetch`, asserting the exact
 * REST calls the backend twin makes (bearer + URL from api-client.test.ts,
 * name-resolved profileId + prompt from the backend create_session case).
 */
...
describe("mcp tools (handler-level, real crypto)", () => {
  ...
  it("create_session over the REAL MoteApi with fetch stubbed hits the backend twin's REST calls", async () => {
    // Task 13 Step 2: clone of the backend create_session assertion, pushed one
    // layer down — no fakeApi, a stubbed global fetch instead (api-client.test
    // pattern), so the full agent-side path (MoteApi.req → fetch) is proven:
    // GET /api/profiles (bearer), then POST /api/sessions with the NAME-resolved
    // profileId and the prompt.
