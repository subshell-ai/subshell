...
// read or write the developer's real ~/.config/mote-agent. Tests that care
// about a pristine (empty) home call newHome() themselves; this only guarantees
// nothing lands in $HOME.
process.env.MOTE_AGENT_HOME = mkdtempSync(join(tmpdir(), "mote-agent-test-"));
...
export function newHome(): string {
  const dir = mkdtempSync(join(tmpdir(), "mote-agent-test-"));
