...
process.env.SUBSHELL_AGENT_HOME = mkdtempSync(join(tmpdir(), "subshell-test-"));
...
process.env.SUBSHELL_AGENT_SKIP_TMUX_CHECK = "1";

/** Fresh empty directory, installed as SUBSHELL_AGENT_HOME; returns its path. */
export function newHome(): string {
  const dir = mkdtempSync(join(tmpdir(), "subshell-test-"));
  process.env.SUBSHELL_AGENT_HOME = dir;
  return dir;
}
