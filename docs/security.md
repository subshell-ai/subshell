...
| `actor` | `cookie` | `system-key` | `subshell-key` | none (rejected on REST) |
| `principal` | `user:<id>` | `user:<systemUserId>` | `sess:<subshellId>` | none |
...
`auth.ts:165-177`), and the bearer path refuses too (`auth-guard.ts:154-171`),
...
and on the SAME key: a disable freezes the account and does not revoke
...
- **Every entry is validated by COMPONENT and stored canonicalized.** Both
  ...
  a host, and a path, query or fragment is refused, and then `URL.origin` is what
...
handshake; see §2, "Disabling an account"): the store drop cannot reach a
