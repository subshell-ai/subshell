...
("Server‪ppa" renders "Serverapp" — node-name spoof), ZWJ/ZWSP/VS16,
...
imports repo-wide — pulls `fast-uri`, 7 high host-confusion advisories, into
the dev tree) and `better-auth-ui@3.2.27` (declared in `apps/server/api`,
zero imports — pulls `@instantdb/react → uuid`). DELETE both, re-run
