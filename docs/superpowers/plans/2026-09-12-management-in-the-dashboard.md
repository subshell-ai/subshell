...
- Execute: `docs/superpowers/plans/2026-09-11-native-reset-both-desktop-apps.md` **Task 4 only** (Subshell Client's reset chain — `node_arm_reset`, `node_reset`) exactly as written there; its **Task 5** (the reset UI) is built as `components/assistant/reset-screen.tsx` in the frame (title *Reset This Mac*, the five deletion rows, the disclosures paragraph, the hostname field, **Reset Everything** destructive / **Cancel** ghost) instead of into the card page.

  **That plan's checkboxes are stale; verified on 2026-09-12.** Its Task 1 (the shared guards) is DONE — `crates/desktop-core/src/reset_guards.rs` exports `path_rules_ok`, `delete_guard_ok`, `is_subshell_socket` and `consent_granted`, and `apps/server/desktop/src-tauri/src/reset.rs` already imports all four; the client's chain uses the same four rather than writing its own. Its Task 2 (the node CLI's `paths` block) is DONE — `subshell status --json` carries `{ configFile, lockFile, dataDir }` when a config loaded and omits the key entirely when none did, which is the authority the client's delete plan is parsed from. Its Task 3 (the server console's entry) is SUPERSEDED — the server's reset screen is Task 22 of this plan. Re-read those three before assuming anything else in that file is outstanding.
