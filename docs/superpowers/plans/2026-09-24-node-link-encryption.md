...
- [ ] **Step 3b (ledger Ruling R2): `frameBytes` must measure binary natively.** `node-ws-handler.ts`'s `frameBytes(raw)` uses `Buffer.byteLength(JSON.stringify(raw))` for anything non-string — a Buffer ciphertext would serialize to a `{"type":"Buffer","data":[…]}` monster and every encrypted frame would read ~66× oversize and close 1009. Amend it: `Buffer`/`Uint8Array`/`ArrayBuffer` → `byteLength` (via `Buffer.from(u8.buffer, u8.byteOffset, u8.length).byteLength` for the first two; `byteLength` for ArrayBuffer); the JSON.stringify branch stays for pre-parsed objects. Pin with a unit test each shape. This is the cap Task 8's ciphertext check depends on.
- [ ] **Step 3c: widen every test fake to the new `NodeSocket` signature.** `node-ws-handler.test.ts`'s `fakeSocket` and the other `send(d: string)` fakes (held-connections, registry tests, scripted-node) must accept `string | Buffer` in the same commit — TS contravariance makes a narrower fake a type error against the widened interface, and `verify-types` catches exactly that.
...
- **mode "legacy"** (row null): text `{t:"register", pub}` (validated 32-byte base64) → `setEncryptPublicKey(id, pub)` → reply text `{t:"register-ok", controlEncryptPublicKey: <server static pub>}` → socket CLOSES normally (the agent reconnects encrypted; do not try to handshake on the same socket — spec §5). Otherwise plaintext is `forwarded` to the existing gates/hold path **with one addition (ledger Ruling R3)**: the legacy row may never accept a `ready`. A below-floor or protocol-mismatched `ready` holds exactly as today (that's what keeps a v13 agent offline-but-updatable and records its `agentVersion`); a `ready` that WOULD pass both gates (version ≥ floor ∧ protocol == NODE_PROTOCOL_VERSION) is held with a NEW reason — `"encryption-required"` — added to `HeldReason` (`node-registry.ts:165`) and the `held` union in `node-view.ts` (chip copy: "node needs to re-pair its encryption identity"; it reads like the below-floor chip because the remedy is the same — the node updates and registers). Rationale: a v14 agent on a pin-less row has exactly one legitimate first frame, `register`; anything else — including a plaintext `ready` wearing 14 — is the downgrade attempt Review Focus #1 names, and the row's lack of a pin is not evidence it may run.
...
- [ ] **Step 1: failing tests at this layer** — (a) handshake row: a full kx→binding sequence over the REAL `handleNodeMessage` entry (fake sockets carrying text and Buffer frames) reaches `ready` handling only after establishment; (b) a `ready` as first frame on a handshake row closes 4410 BEFORE `parseNodeEvent`/`applyReady` runs (the existing test that asserts identity persists before the gates must still pass — classification uses the upgrade-stashed row facts, no second query, so persistence is untouched); (c) legacy row + register frame handled and socket closed after register-ok; (d) legacy row + today's plaintext frames still flow to the existing gates (the full held-path suite for below-floor/protocol mismatch stays green — it's the same code, now reached via `forwarded`); (e) **R3's case (ledger Ruling R3)**: legacy row + a plaintext `ready` with protocol 14 and an agentVersion at/above the floor → HELD with `reason: "encryption-required"` (never `applyReady`-online), and the `held` view exposes the new reason; a below-floor plaintext ready still holds with `below-floor` as today.
