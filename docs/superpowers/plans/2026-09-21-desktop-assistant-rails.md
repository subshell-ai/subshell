...
- [ ] Step 2: `App` composes `Rail` from `@internal/assistant`; the tray's `desktop-screen` events (`about`, `update`) select their section (the existing override state becomes the rail's active id — same semantics, no new command).
