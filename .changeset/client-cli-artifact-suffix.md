...
`subshell-cli-<triple>` (with the `.sha256` sidecar following the binary's name,
as always). These are both the `client-vX.Y.Z` release assets and the files
...
release:client` with `SUBSHELL_NODE_ARTIFACTS_DIR` pointed at the directory the
server serves). Until it does, every agent download 404s: the server now looks
for `subshell-cli-<triple>` and the old files are still on disk under the old
names.
