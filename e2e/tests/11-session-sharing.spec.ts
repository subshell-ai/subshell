...
/**
 * End-to-end session sharing (spec 2026-08-31 §4) with two real users.
 *
 * One context can't hold two principals, so — like spec 10 — this drives
 * `browser.newContext` directly: an admin context mints a member + a session,
...
test("sharing: view is read-only, edit manages, private sessions are invisible", async ({ browser }) => {
  ...
  const mkSession = async (name: string): Promise<string> => {
    const res = await admin.request.post("/api/sessions", { data: { profileId, workingDir: "/tmp", name } });
    expect(res.ok(), await res.text()).toBe(true);
    return ((await res.json()) as { id: string }).id;
  };
  const sharedId = await mkSession("shared-target");
  const privateId = await mkSession("private-target");
  ...
  try {
    ...
    expect((await memberBrowser.request.get(`/api/sessions/${privateId}`)).status()).toBe(404);

    // Share the target at VIEW. The member now sees it, read-only.
    expect(
      (
        await admin.request.put(`/api/sessions/${sharedId}/shares`, {
          data: { shares: [{ granteeUserId: null, permission: "view" }] },
        })
      ).ok(),
    ).toBe(true);
    const asView = await (await memberBrowser.request.get(`/api/sessions/${sharedId}`)).json();
    expect(asView.access).toBe("view");

    await page.goto(`/sessions/${sharedId}`);
    ...
    const asEdit = await (await memberBrowser.request.get(`/api/sessions/${sharedId}`)).json();
    expect(asEdit.access).toBe("edit");
    await page.goto(`/sessions/${sharedId}`);
    await expect(page.getByRole("button", { name: /^Actions for / })).toBeVisible({ timeout: SPAWN_TIMEOUT });

    // Revoke entirely: the session vanishes from the member again (404).
    await admin.request.put(`/api/sessions/${sharedId}/shares`, { data: { shares: [] } });
    expect((await memberBrowser.request.get(`/api/sessions/${sharedId}`)).status()).toBe(404);
  } finally {
    for (const id of [sharedId, privateId]) await admin.request.delete(`/api/sessions/${id}`);
